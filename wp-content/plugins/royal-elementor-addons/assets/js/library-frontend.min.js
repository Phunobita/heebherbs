( function( $ ) {
	...
	var WprLibraryTmpls = {
		...
		renderPopupContent: function( previewIframe, tab, scrolling = false ) {
			...
			// Render Preview
				...
				previewIframe.find( '.wpr-tplib-sidebar input' ).on( 'keyup', function() {
					...
					searchTimeout = setTimeout(function() {
						...
						if ( 'pages' === tab ) {
							previewIframe.find('.wpr-tplib-template-gird').trigger('scroll').scrollTop(1000).trigger('scroll').scrollTop(0).trigger('scroll');
			...
			if ( 'pages' === tab ) {
				setTimeout(function(){
					WprLibraryTmpls.renderTemplatesGridContent(previewIframe);
				}, 2000);
			}
			
		},

		renderTemplatesGridContent: function( previewIframe ) {
			let page = 1;
			let loading = false;

			previewIframe.find('.wpr-tplib-template-gird').on('scroll', function() {
				// console.log('scrolling...');

				if ( ($(this).scrollTop() + $(this).outerHeight()) >= $(document).height() - 100 ) {
					if (loading) return;
